...
template <typename T, typename I>
Status FSumOp<T, I>::Init(OpKernelConstruction* ctx) {
  XDL_CHECK_STATUS(ctx->GetAttr("average", &average_));
  return Status::Ok();
}

template <typename T, typename I>
Status FSumOp<T, I>::Compute(OpKernelContext* ctx) {
  ...
  TensorShape out_shape({seg_size, stats_dim});
  if (group.Shape().NumElements() == 0) {
    pgrp = pseg;
  } else {
    grp_size = group.Shape().NumElements();
  ...
  for (size_t i = 0; i < id_size; ++i) {
    ...
    size_t grp_width = (grp_idx == 0) ? pgrp[grp_idx]
                                      : pgrp[grp_idx] - pgrp[grp_idx - 1];
    if (grp_width == 0) continue;
    const T* src = pstats + pidx[i] * stats_dim;
    T* dst = pout + grp_idx * stats_dim;
...
#define REGISTER_KERNEL(T, I)                \
  XDL_REGISTER_KERNEL(FSum, FSumOp<T, I>) \
  .Device("CPU")                             \
  .AttrDataType<T>("dtype")                  \
  .AttrDataType<I>("itype")
