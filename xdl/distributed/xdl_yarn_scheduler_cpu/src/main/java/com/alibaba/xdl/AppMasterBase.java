...
public class AppMasterBase {
  ...
  private String genCmd(String xdlUser, String workType, int workIndex, String config, String xdlZKRoot,
    ...
    String command = "$JAVA_HOME/bin/java" + " -Xmx256M" + container + " -c=" + config + " -j=" + workType + " -i="
        ...
        + volumeDirInHdfs + " -cpuset=_CPU_LIST_" + (port == null ? "" : (" -xp=" + port)) 
  ...
  private boolean matchesResource(Resource request, Resource response) {
    if (response.getVirtualCores() == request.getVirtualCores() && response.getMemorySize() >= request.getMemorySize()) {
      return true;
    }
    return false;
  }
  ...
  private void initResource() throws Exception {
    ...
    this.psNumber = this.schedulerConf.ps.instance_num;
    {
      ...
      LOG.info("Worker container capability:[CPU: {}, Memory: {} MB", cpuCores, memory);
    }
    if (this.schedulerConf.extend_role != null) {
      ...
      while (iter.hasNext()) {
        ...
        LOG.info("{} container capability:[CPU: {}, Memory: {} MB", key, cpuCores, memory);
      }
    }
    {
      ...
      LOG.info("PS container capability:[CPU: {}, Memory: {} MB", cpuCores, memory);
    }
  ...
  private void createRMClient() throws Exception {
    ...
    LOG.info("ApplicationMaster max memory [{}] max cpu cores [{}]", maxResource.getMemorySize(),
