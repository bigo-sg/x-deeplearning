...
public class ContainerBase {
  ...
  protected String genCmd() throws Exception {
    ...
    if (this.schedulerConf.docker != null) {
      ...
      if (this.schedulerConf.docker.run_params != null) {
        run_params = this.schedulerConf.docker.run_params.trim();
      }
    }
    ...
    DockerCmdBuilder builder = DockerCmdBuilder.newBuilder().withUser(this.yarnUser).withScript(worker_script)
        ...
        .withDockerContainerName(dockerName)
        ...
        .withCpuCores(cpuCores).withMemoryLimit(memLimit)
        .withZkAddr("zfs://" + zookeeperConnStr + "/psplus/" + yarnAppId).withSchedulerConf(schedulerConf)
        .withAppId(yarnAppId).withVpMethod(vp_method).withMetaDir(meta_dir).withCpuSet(cpuset).withRunParams(run_params);
