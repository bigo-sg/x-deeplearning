...
#ifndef PS_COMMON_STRING_UTILS_H
#define PS_COMMON_STRING_UTILS_H
...
public:
    ...
    static bool GetValueFromMap(const std::map<std::string, std::string>& params, 
...
inline std::string StringUtils::toString(const double &x, int32_t precision) {
    StringStreamWrapper oss;
...
template<typename T>
inline std::string StringUtils::toString(const std::vector<T> &x, const std::string &delim) {
    StringStreamWrapper oss; 
...
template<typename T>
inline std::string StringUtils::toString(const std::vector<std::vector<T> > &x, 
